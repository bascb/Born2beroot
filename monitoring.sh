...
physcpu=$(grep "physical id" /proc/cpuinfo | sort | uniq | wc -l)
...
used_disk=$(df -Bm | grep '^/dev/' | grep -v '/boot$' | awk '{fdisk += $3} END {print fdisk}')
...
cpu_usage_percent=$(top -bn1 | grep '^%Cpu' | awk '{printf("%.1f%%"), $2}')
...
wall "  #Architecture: $architecture
        ...
        #vCPU: $virtcpu
        #Memory Usage: ${used_ram}MB/${total_ram}MB ($ram_usage_percent%)
        #Disk Usage: ${used_disk}MB/${total_disk}GB ($disk_usage_percent%)
